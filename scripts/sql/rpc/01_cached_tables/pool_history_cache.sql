DROP TABLE IF EXISTS {{SCHEMA}}.pool_history_cache;

CREATE TABLE {{SCHEMA}}.pool_history_cache (
  pool_id varchar,
  epoch_no int8 NULL,
...
COMMENT ON TABLE {{SCHEMA}}.pool_history_cache IS 'A history of pool performance including blocks, delegators, active stake, fees AND rewards';

CREATE OR REPLACE FUNCTION {{SCHEMA}}.pool_history_cache_update(_epoch_no_to_insert_from bigint DEFAULT NULL)
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  IF (
    ...
    WHERE state = 'active' AND query ILIKE '%{{SCHEMA}}.pool_history_cache_update%'
  ...
  DELETE FROM {{SCHEMA}}.pool_history_cache
  WHERE epoch_no >= _epoch_no_to_insert_from;
  INSERT INTO {{SCHEMA}}.pool_history_cache (
  WITH
    blockcounts AS (
      SELECT
        sl.pool_hash_id,
    ...
    activeandfees AS (
      SELECT
        ...
        (act.amount / (
          ...
          FROM {{SCHEMA}}.epoch_active_stake_cache AS easc
        ...
        ROUND(
          (act.amount / (
            ...
            FROM {{SCHEMA}}.totals (act.epoch_no)
            ) * 100
          ), 2
        ) AS saturation_pct
      FROM {{SCHEMA}}.pool_active_stake_cache AS act
...
COMMENT ON FUNCTION {{SCHEMA}}.pool_history_cache_update IS 'Internal function to update pool history for data FROM specified epoch until current-epoch-minus-one. Invoke WITH non-empty param for initial population, WITH empty for subsequent updates'; --noqa: LT01
