CREATE OR REPLACE PROCEDURE {{SCHEMA}}.update_newly_registered_accounts_stake_distribution_cache()
LANGUAGE plpgsql
AS $$
BEGIN
  IF (
  ...
  ELSIF (
    ...
    CALL {{SCHEMA}}.kill_queries_partial_match('{{SCHEMA}}.stake_distribution_cache_update_check(');
  END IF;
      ...
      WHERE
        ...
        AND NOT EXISTS (
          ...
          WHERE epoch_stake.epoch_no = (
            SELECT last_value::integer FROM {{SCHEMA}}.control_table
  ...
  INSERT INTO {{SCHEMA}}.stake_distribution_cache
    ...
    FROM newly_registered_accounts AS nra,
      LATERAL {{SCHEMA}}.account_info(array[nra.stake_address]) AS ai
    ...
    DELETE FROM {{SCHEMA}}.stake_distribution_cache
