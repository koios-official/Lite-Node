CREATE TABLE IF NOT EXISTS {{SCHEMA}}.stake_distribution_cache (
...
CREATE OR REPLACE PROCEDURE {{SCHEMA}}.update_stake_distribution_cache()
LANGUAGE plpgsql
AS $$
...
BEGIN
  ...
  SELECT (last_value::integer - 2)::integer INTO _active_stake_epoch FROM {{SCHEMA}}.control_table
  ...
  WITH
    ...
    account_active_stake AS (
      SELECT
        awdp.stake_address_id,
        acsc.amount
      FROM {{SCHEMA}}.account_active_stake_cache AS acsc
  ...
  -- INSERT QUERY START
  INSERT INTO {{SCHEMA}}.stake_distribution_cache
    SELECT
      awdp.stake_address,
      pi.pool_id,
...
-- HELPER FUNCTION: {{SCHEMA}}.stake_distribution_cache_update_check
...
CREATE OR REPLACE FUNCTION {{SCHEMA}}.stake_distribution_cache_update_check()
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  IF (
    ...
    WHERE state = 'active'
      AND query ILIKE '%{{SCHEMA}}.stake_distribution_cache_update_check(%'
      AND datname = (
  ...
  ELSIF (
    ...
    FROM pg_stat_activity
    WHERE state = 'active'
      AND query ILIKE '%{{SCHEMA}}.update_newly_registered_accounts_stake_distribution_cache(%'
      AND datname = (
        SELECT current_database()
      )
  ) THEN
  ...
  Raise NOTICE 'Last stake distribution update was % blocks ago...',
    ...
    IF (_last_update_block_diff >= 180
      ...
      CALL {{SCHEMA}}.update_stake_distribution_cache ();
    ELSE
...
DROP INDEX IF EXISTS {{SCHEMA}}.idx_pool_id;
CREATE INDEX idx_pool_id ON {{SCHEMA}}.stake_distribution_cache (pool_id);
