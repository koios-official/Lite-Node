...
CREATE TABLE IF NOT EXISTS {{SCHEMA}}.stake_snapshot_cache (
  addr_id integer,
  pool_id integer,
  amount numeric,
  epoch_no bigint,
...
CREATE INDEX IF NOT EXISTS _idx_pool_id ON {{SCHEMA}}.stake_snapshot_cache (pool_id);
CREATE INDEX IF NOT EXISTS _idx_addr_id ON {{SCHEMA}}.stake_snapshot_cache (addr_id);

CREATE OR REPLACE PROCEDURE {{SCHEMA}}.capture_last_epoch_snapshot()
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  IF (
    ...
    SELECT COUNT(pid) > 1
      ...
      WHERE state = 'active'
        AND query ILIKE '%{{SCHEMA}}.capture_last_epoch_snapshot(%'
        AND datname = (
  ...
  IF NOT EXISTS (
    SELECT i_last_tx_id FROM {{SCHEMA}}.epoch_info_cache
  ...
  IF EXISTS (
    SELECT FROM {{SCHEMA}}.stake_snapshot_cache
      WHERE epoch_no = _previous_epoch_no
  ) THEN
    RETURN;
  END IF;
...
/* Registered and delegated accounts to be captured (have epoch_stake entries for baseline) */
        ...
        AND NOT EXISTS (
          ...
          WHERE pu.hash_id = pr.hash_id
            ...
            AND pu.registered_tx_id <= (
              SELECT i_last_tx_id
              FROM {{SCHEMA}}.epoch_info_cache AS eic
  ...
  WITH
    ...
    INSERT INTO {{SCHEMA}}.stake_snapshot_cache
      SELECT
  ...
  INSERT INTO {{SCHEMA}}.epoch_active_stake_cache
    SELECT
      ...
      SUM(amount)
    FROM {{SCHEMA}}.stake_snapshot_cache
    WHERE epoch_no = _previous_epoch_no
  ...
  INSERT INTO {{SCHEMA}}.pool_active_stake_cache
    SELECT
      ...
      _previous_epoch_no + 2,
      SUM(ssc.amount)
    FROM {{SCHEMA}}.stake_snapshot_cache AS ssc
  ...
  DELETE FROM {{SCHEMA}}.stake_snapshot_cache
