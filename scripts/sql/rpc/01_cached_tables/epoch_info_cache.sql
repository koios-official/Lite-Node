CREATE TABLE IF NOT EXISTS {{SCHEMA}}.epoch_info_cache (
...
COMMENT ON TABLE {{SCHEMA}}.epoch_info_cache IS 'Contains detailed info for epochs including protocol parameters';

CREATE OR REPLACE FUNCTION {{SCHEMA}}.epoch_info_cache_update(
...
BEGIN
  ...
  IF (
    ...
    WHERE state = 'active'
      AND query ILIKE '%{{SCHEMA}}.epoch_info_cache_update%'
  ...
  IF _epoch_no_to_insert_from IS NULL THEN
    ...
    IF _latest_epoch_no_in_cache = 0 THEN
      ...
      PERFORM {{SCHEMA}}.epoch_info_cache_update(0);
      RETURN;
    END IF;
    ...
    IF _curr_epoch = _latest_epoch_no_in_cache THEN
      ...
      PERFORM {{SCHEMA}}.update_latest_epoch_info_cache(_curr_epoch, _latest_epoch_no_in_cache);
      RETURN;
    END IF;
    ...
    PERFORM {{SCHEMA}}.update_total_rewards_epoch_info_cache(_latest_epoch_no_in_cache - 1);
  ...
  DELETE FROM {{SCHEMA}}.epoch_info_cache
  ...
  INSERT INTO {{SCHEMA}}.epoch_info_cache
...
CREATE OR REPLACE FUNCTION {{SCHEMA}}.update_latest_epoch_info_cache(_curr_epoch bigint, _epoch_no_to_update bigint)
RETURNS void
LANGUAGE plpgsql
AS $$
BEGIN
  ...
  IF _curr_epoch <> _epoch_no_to_update THEN
    UPDATE {{SCHEMA}}.epoch_info_cache
...
CREATE OR REPLACE FUNCTION {{SCHEMA}}.update_total_rewards_epoch_info_cache(_epoch_no_to_update bigint)
RETURNS void
LANGUAGE plpgsql
AS $$
BEGIN
