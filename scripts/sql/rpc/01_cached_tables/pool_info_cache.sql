DROP TABLE IF EXISTS {{SCHEMA}}.pool_info_cache;

CREATE TABLE {{SCHEMA}}.pool_info_cache (
...
COMMENT ON TABLE {{SCHEMA}}.pool_info_cache IS 'A summary of all pool parameters and updates';

CREATE OR REPLACE FUNCTION {{SCHEMA}}.pool_info_insert(
  _update_id bigint,
  _tx_id bigint,
  _hash_id bigint,
...
BEGIN
  ...
  INSERT INTO {{SCHEMA}}.pool_info_cache (
    tx_id,
    update_id,
    tx_hash,
    block_time,
...
COMMENT ON FUNCTION {{SCHEMA}}.pool_info_insert IS 'Internal function to insert a single pool update'; -- noqa: LT01

CREATE OR REPLACE FUNCTION {{SCHEMA}}.pool_info_retire_status()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
  UPDATE {{SCHEMA}}.pool_info_cache
...
COMMENT ON FUNCTION {{SCHEMA}}.pool_info_retire_status IS 'Internal function to update pool_info_cache with new retire status based ON epoch switch'; -- noqa: LT01

CREATE OR REPLACE FUNCTION {{SCHEMA}}.pool_info_retire_update()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  END IF;
  SELECT COALESCE(MAX(tx_id), 0) INTO _latest_pool_update_tx_id FROM {{SCHEMA}}.pool_info_cache AS pic WHERE pic.pool_hash_id = _pool_hash_id;
...
COMMENT ON FUNCTION {{SCHEMA}}.pool_info_retire_update IS 'Internal function to update pool_info cache table based ON insert/delete ON pool_retire table'; -- noqa: LT01

CREATE OR REPLACE FUNCTION {{SCHEMA}}.pool_info_update()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  IF (tg_table_name = 'pool_update') THEN
    IF (tg_op = 'INSERT') THEN
      PERFORM {{SCHEMA}}.pool_info_insert(
        new.id,
...
COMMENT ON FUNCTION {{SCHEMA}}.pool_info_update IS 'Internal function to insert/delete pool updates in pool_info cache table'; -- noqa: LT01
...
FOR EACH ROW EXECUTE FUNCTION {{SCHEMA}}.pool_info_update();
...
FOR EACH ROW EXECUTE FUNCTION {{SCHEMA}}.pool_info_retire_update();
...
FOR EACH ROW EXECUTE FUNCTION {{SCHEMA}}.pool_info_retire_status();
...
DECLARE
  ...
  rec RECORD;
BEGIN
  SELECT COALESCE(MAX(tx_id), 0) INTO _latest_pool_info_tx_id FROM {{SCHEMA}}.pool_info_cache;
  ...
  CREATE INDEX IF NOT EXISTS idx_tx_id ON {{SCHEMA}}.pool_info_cache (tx_id);
  CREATE INDEX IF NOT EXISTS idx_pool_id_bech32 ON {{SCHEMA}}.pool_info_cache (pool_id_bech32);
  CREATE INDEX IF NOT EXISTS idx_pool_hash_id ON {{SCHEMA}}.pool_info_cache (pool_hash_id);
  CREATE INDEX IF NOT EXISTS idx_pool_status ON {{SCHEMA}}.pool_info_cache (pool_status);
  CREATE INDEX IF NOT EXISTS idx_meta_id ON {{SCHEMA}}.pool_info_cache (meta_id);
END;
$$;
