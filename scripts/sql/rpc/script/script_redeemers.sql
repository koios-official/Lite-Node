CREATE OR REPLACE FUNCTION {{SCHEMA}}.script_redeemers(_script_hash text)
RETURNS TABLE (
  script_hash text,
  redeemers jsonb
)
...
COMMENT ON FUNCTION {{SCHEMA}}.script_redeemers IS 'Get all redeemers for a given script hash.'; --noqa: LT01
