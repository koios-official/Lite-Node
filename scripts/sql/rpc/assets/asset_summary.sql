CREATE OR REPLACE FUNCTION {{SCHEMA}}.asset_summary(_asset_policy text, _asset_name text DEFAULT '')
RETURNS TABLE (
  policy_id text,
  asset_name text,
...
COMMENT ON FUNCTION {{SCHEMA}}.asset_summary IS 'Get the summary of an asset (total transactions exclude minting/total wallets include only wallets with asset balance)'; -- noqa: LT01
