CREATE OR REPLACE FUNCTION {{SCHEMA}}.asset_address_list(_asset_policy text, _asset_name text DEFAULT '')
RETURNS TABLE (
...
BEGIN
  ...
  SELECT COUNT(ma_id) INTO _isatoc FROM {{SCHEMA}}.asset_tx_out_cache
...
COMMENT ON FUNCTION {{SCHEMA}}.asset_address_list IS 'DEPRECATED!! Use asset_addresses instead.'; -- noqa: LT01
