CREATE OR REPLACE FUNCTION {{SCHEMA}}.epoch_summary_corrections_update()
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
  ...
  last_epoch_checked bigint = coalesce((SELECT last_value::bigint FROM {{SCHEMA}}.control_table WHERE key = 'last_epoch_summary_data_checked'), -1);
BEGIN
  ...
  IF last_epoch_checked < 0 THEN
    ...
    INSERT INTO {{SCHEMA}}.control_table values('last_epoch_summary_data_checked', 0, null);
  END IF;
  ...
  END LOOP;
  UPDATE {{SCHEMA}}.control_table SET last_value = latest_epoch::text WHERE key = 'last_epoch_summary_data_checked';
END;
$$;
