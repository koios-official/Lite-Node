CREATE OR REPLACE FUNCTION {{SCHEMA}}.epoch_params(_epoch_no numeric DEFAULT NULL)
RETURNS TABLE (
  epoch_no word31type,
...
BEGIN
  RETURN QUERY
    ...
    LEFT JOIN {{SCHEMA}}.epoch_info_cache AS ei ON ei.epoch_no = ep.epoch_no
...
COMMENT ON FUNCTION {{SCHEMA}}.epoch_params IS 'Get the epoch parameters, all epochs if no epoch specified'; -- noqa: LT01
