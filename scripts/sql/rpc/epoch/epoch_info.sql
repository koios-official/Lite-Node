CREATE OR REPLACE FUNCTION {{SCHEMA}}.epoch_info(_epoch_no numeric DEFAULT NULL, _include_next_epoch boolean DEFAULT FALSE)
RETURNS TABLE (
  epoch_no word31type,
  out_sum text,
  fees text,
...
AS $$
DECLARE
  shelley_epoch_duration numeric := (select epochlength::numeric * slotlength::numeric AS epochduration FROM {{SCHEMA}}.genesis);
  shelley_ref_epoch numeric := (select (ep.epoch_no::numeric + 1) FROM epoch_param ep ORDER BY ep.epoch_no LIMIT 1);
  shelley_ref_time numeric := (select ei.i_first_block_time FROM {{SCHEMA}}.epoch_info_cache ei where ei.epoch_no = shelley_ref_epoch);
BEGIN
  RETURN QUERY
  SELECT
    ei.epoch_no,
  ...
  FROM {{SCHEMA}}.epoch_info_cache AS ei
  LEFT JOIN {{SCHEMA}}.epoch_active_stake_cache AS eas ON eas.epoch_no = ei.epoch_no
  WHERE
...
COMMENT ON FUNCTION {{SCHEMA}}.epoch_info IS 'Get the epoch information, all epochs if no epoch specified. If _include_next_epoch is set to true, also return active stake calculation for next epoch if available'; -- noqa: LT01
