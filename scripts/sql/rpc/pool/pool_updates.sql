CREATE OR REPLACE FUNCTION {{SCHEMA}}.pool_updates(_pool_bech32 text DEFAULT NULL)
RETURNS TABLE (
  tx_hash text,
  block_time integer,
...
COMMENT ON FUNCTION {{SCHEMA}}.pool_updates IS 'Return all pool_updates for all pools or only updates for specific pool if specified'; -- noqa: LT01
