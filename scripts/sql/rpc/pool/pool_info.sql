CREATE OR REPLACE FUNCTION {{SCHEMA}}.pool_info(_pool_bech32_ids text [])
RETURNS TABLE (
...
BEGIN
  ...
  SELECT FLOOR(supply::bigint / (
    ...
    ))::bigint INTO _saturation_limit FROM {{SCHEMA}}.totals(_epoch_no);
  RETURN QUERY
    WITH
      _all_pool_info AS (
    ...
    LEFT JOIN LATERAL(
      ...
      FROM {{SCHEMA}}.pool_active_stake_cache AS pasc
    ...
    LEFT JOIN LATERAL(
      SELECT
        ...
        END AS pledge
      FROM {{SCHEMA}}.stake_distribution_cache AS sdc
      WHERE sdc.pool_id = api.pool_id_bech32
    ) AS live ON TRUE;
END;
$$;

COMMENT ON FUNCTION {{SCHEMA}}.pool_info IS 'Current pool status and details for a specified list of pool ids'; --noqa: LT01
