CREATE OR REPLACE FUNCTION {{SCHEMA}}.utxo_info(_utxo_refs text [], _extended boolean DEFAULT false)
RETURNS TABLE (
  tx_hash text,
  tx_index smallint,
  address text,
...
COMMENT ON FUNCTION {{SCHEMA}}.utxo_info IS 'Get details for requested UTxO arrays (UTxOs accepted in <hash>#<index> format).'; -- noqa: LT01
