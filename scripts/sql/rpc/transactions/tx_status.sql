CREATE OR REPLACE FUNCTION {{SCHEMA}}.tx_status(_tx_hashes text [])
RETURNS TABLE (
  tx_hash text,
...
COMMENT ON FUNCTION {{SCHEMA}}.tx_status IS 'Returns number of blocks that were created since the block containing a transactions with a given hash'; -- noqa: LT01
