...
CREATE OR REPLACE FUNCTION {{SCHEMA}}v0.address_assets(_addresses text [])
RETURNS TABLE (
  address varchar,
  asset_list jsonb
)
...
CREATE OR REPLACE FUNCTION {{SCHEMA}}v0.address_info(_addresses text [])
RETURNS TABLE (
  address varchar,
  balance text,
...
BEGIN
  RETURN QUERY
    SELECT * FROM {{SCHEMA}}.address_info(_addresses);
END;
$$;

CREATE OR REPLACE FUNCTION {{SCHEMA}}v0.address_txs(_addresses text [], _after_block_height integer DEFAULT 0)
RETURNS TABLE (
  tx_hash text,
  epoch_no word31type,
...
BEGIN
  RETURN QUERY
    SELECT * FROM {{SCHEMA}}.address_txs(_addresses, _after_block_height);
END;
$$;

CREATE OR REPLACE FUNCTION {{SCHEMA}}v0.credential_txs(_payment_credentials text [], _after_block_height integer DEFAULT 0)
RETURNS TABLE (
  tx_hash text,
  epoch_no word31type,
  block_height word31type,
...
BEGIN
  RETURN QUERY
    SELECT * FROM {{SCHEMA}}.credential_txs(_payment_credentials,_after_block_height);
END;
$$;

CREATE OR REPLACE FUNCTION {{SCHEMA}}v0.credential_utxos(_payment_credentials text [])
RETURNS TABLE (
  tx_hash text,
  tx_index smallint,
  value text
