...
CREATE OR REPLACE FUNCTION {{SCHEMA}}v0.epoch_block_protocols(_epoch_no numeric DEFAULT NULL)
RETURNS TABLE (
...
BEGIN
  RETURN QUERY
    SELECT * FROM {{SCHEMA}}.epoch_block_protocols(_epoch_no);
END;
$$;

CREATE OR REPLACE FUNCTION {{SCHEMA}}v0.epoch_info(_epoch_no numeric DEFAULT NULL, _include_next_epoch boolean DEFAULT FALSE)
RETURNS TABLE (
  epoch_no word31type,
  out_sum text,
  fees text,
...
BEGIN
  RETURN QUERY
  SELECT * FROM {{SCHEMA}}.epoch_info(_epoch_no, _include_next_epoch);
END;
$$;

CREATE OR REPLACE FUNCTION {{SCHEMA}}v0.epoch_params(_epoch_no numeric DEFAULT NULL)
RETURNS TABLE (
  epoch_no word31type,
...
BEGIN
  RETURN QUERY
    SELECT * FROM {{SCHEMA}}.epoch_params(_epoch_no);
END;
$$;
