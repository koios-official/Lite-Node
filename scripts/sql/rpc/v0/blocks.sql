...
CREATE OR REPLACE FUNCTION {{SCHEMA}}v0.block_info(_block_hashes text [])
RETURNS TABLE (
  hash text,
  epoch_no word31type,
  abs_slot word63type,
...
BEGIN
  RETURN QUERY
  SELECT * FROM {{SCHEMA}}.block_info(_block_hashes);
END;
$$;

CREATE OR REPLACE FUNCTION {{SCHEMA}}v0.block_txs(_block_hashes text [])
RETURNS TABLE (
  block_hash text,
  tx_hashes text []
)
