...
CREATE OR REPLACE FUNCTION {{SCHEMA}}v0.datum_info(_datum_hashes text [])
RETURNS TABLE (
  hash text,
  value jsonb,
  bytes text
...
CREATE OR REPLACE FUNCTION {{SCHEMA}}v0.native_script_list()
RETURNS TABLE (
  script_hash text,
...
CREATE OR REPLACE FUNCTION {{SCHEMA}}v0.plutus_script_list()
RETURNS TABLE (
  script_hash text,
  creation_tx_hash text
)
...
CREATE OR REPLACE FUNCTION {{SCHEMA}}v0.script_redeemers(_script_hash text)
RETURNS TABLE (
  script_hash text,
  redeemers jsonb
)
...
BEGIN
  RETURN QUERY
  SELECT * FROM {{SCHEMA}}.script_redeemers(_script_hash);
END;
$$;
